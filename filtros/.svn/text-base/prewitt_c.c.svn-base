...
void prewitt_c (
	...
	while(itFilas < cantFilas-1){ 
			...
			while(itColumnas < cantColumnas){
				...
				if (itColumnas == 0 || itColumnas == cantColumnas -1) {gx = 0; gy = 0;}
				else {			
				...
				gy = *(itSrc -src_row_size - 1 ) +  (*(itSrc - src_row_size)) + *(itSrc - src_row_size + 1)
					-*(itSrc +src_row_size - 1 ) -  (*(itSrc + src_row_size)) - *(itSrc + src_row_size + 1);
